export default async function handler(req, res) {
  ...
  const AUDIENCE_DESC = {
    ...
    crush:
      "someone they have a crush on — be confident and naturally charming, not desperate or cringe",
    partner: "their romantic partner / boyfriend / girlfriend / bae",
    family: "a family member — warm and appropriate",
    stranger:
      "someone they don't know well yet — friendly but not overly familiar",
    colleague:
      "a work colleague or classmate — professional but still friendly",
  ...
  const TONE_DESC = {
    casual:
      "Completely relaxed and natural, like texting a close mate. Everyday language, no formality at all.",
    romantic:
      "Warm, heartfelt and genuinely sweet. Show real interest. Playful but sincere — not cringe or try-hard. Make it feel special.",
    funny:
      "Playful and humorous. A witty observation or light joke that fits naturally. Make them laugh or at least smile.",
    uk_slang:
      'Authentic British/UK street talk — words like "fam", "innit", "bare", "peng", "wagwan", "you good", "mandem", "no cap", "it\'s giving", "bruv", "allow it", "on ones" used naturally where they fit. NOT forced.',
    supportive:
      "Warm, empathetic and genuinely caring. Acknowledge their feelings first before anything else. No preaching or empty positivity.",
    formal:
      "Polite, respectful and clear. Slightly professional — good for people they want to impress or don't know well.",
    flirty:
      "Playful, confident and lightly flirtatious. Keep them interested without being overwhelming. A little mystery goes a long way.",
  ...
  const PURPOSE_DESC = {
    check_in:
      "Just checking in on them and seeing how they are doing — natural, not forced",
    compliment:
      "Giving them a genuine compliment that feels specific and real, not generic",
    apologize:
      "Apologising for something sincerely — own it without being overly dramatic",
    interest:
      "Showing interest in them as a person and keeping the conversation going naturally",
    comfort:
      "Comforting or supporting them through something difficult — presence over advice",
    invite:
      "Inviting them somewhere or suggesting they hang out — confident not desperate",
    reconnect:
      "Reconnecting after not talking for a while — natural, no awkward energy",
    respond: "Replying directly and thoughtfully to what they said",
  };

  const nameLine = personName ? `Their name is ${personName}.` : "";
  const handleLine = personHandle
    ? `Their social media handle is @${personHandle}.`
    : "";
  const audienceLine = AUDIENCE_DESC[audience] || AUDIENCE_DESC.friend;
  const toneLine = TONE_DESC[tone] || TONE_DESC.casual;
  const purposeLine =
    PURPOSE_DESC[purpose] || "Send a message that feels natural and genuine";

  // Format past conversation for the prompt
  let pastSection = "";
  if (pastMessages && pastMessages.trim().length > 5) {
    pastSection = `\n\nPREVIOUS CONVERSATION HISTORY:\n---\n${pastMessages.trim()}\n---\nMatch the tone already established. Reference things already discussed naturally.`;
  }
  if (pastImage) {
    pastSection +=
      "\n\nThe user has also uploaded a screenshot of their conversation. Use it to understand the dynamic, tone, and context between them.";
  }
...
${
  context
    ? `WHAT THE STORY IS ABOUT (user's description): "${context}"`
    : "No description given — reply based on what you see in the image."
}
${pastConvoSection}

${
  image
    ? "Look at the image carefully. Understand the mood, place, activity, or emotion it shows."
    : ""
}
...
— Uses ${
      personName
        ? personName + "'s name naturally if it fits"
...
${
  context
    ...
    : "No background — create a natural opener that could work for anyone."
}
${pastConvoSection}
...
Write a complete, genuine reply — 1 to 4 sentences depending on what the situation needs. Don't cut it short artificially. Return ONLY the message. No quotes around it. No explanation or preamble.`;
  ...
  try {
    ...
    // Add past chat screenshot if provided
    if (pastImage) {
      const pastBase64 = pastImage.split(",")[1];
      const pastMime = pastImage.split(";")[0].split(":")[1];
      parts.unshift({
        inline_data: { mime_type: pastMime, data: pastBase64 },
      });
    }
