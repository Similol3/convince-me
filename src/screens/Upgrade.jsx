  ...
  async function handleCancel() {
    ...
    try {
      ...
      if (data.success) {
        ...
        setError(
          data.error || "Could not cancel subscription. Please try again."
        );
      }
    } catch (err) {
  ...
  if (user?.is_pro) {
    return (
      ...
      >
        ...
        {(() => {
          const hoursSince = user.last_payment_at
            ? (Date.now() - new Date(user.last_payment_at).getTime()) / 3600000
            : 999;
          const eligibleForRefund = hoursSince <= 24;

          if (!eligibleForRefund || cancelled) return null;
          ...
          return (
            <button
              onClick={async () => {
                ...
                if (data.success) {
                  alert(
                    "Refund request submitted. We'll process it within 24-48 hours."
                  );
                } else {
                  alert(data.error || "Could not submit refund request.");
                }
              }}
              style={{
        ...
        {showCancelConfirm && (
          ...
          >
            ...
            >
              ...
              >
                You'll keep all Pro features until the end of your current
                billing period. No refunds for the current period.
              </p>
              <div
      ...
      <div style={{ display: "flex", gap: 10 }}>
        {Object.entries(PRICING).map(([key, p]) => (
          <button
            ...
            style={{
              flex: 1,
              background: plan === key ? gr() : C.glass,
