    ...
    <div style={{
      ...
      <div style={{ textAlign: 'center', paddingTop: 8 }}>
        <div style={{
          ...
          background: gr(), margin: '0 auto 12px',
          display: 'flex', alignItems: 'center',
          justifyContent: 'center', fontSize: 32, overflow: 'hidden',
        }}>
          {stats.avatar_image
            ? <img src={stats.avatar_image} alt="avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            : (avatar || '🎲')}
        </div>
      ...
      {stats.is_guest && (
        <div style={{
          ...
          <div style={{ fontSize: 12, color: C.sub, marginBottom: 14, lineHeight: 1.5 }}>
            Create a free account to save your decisions, appear on the leaderboard, and unlock Pro features.
          </div>
      ...
      <div style={{
        ...
        <div style={{ height: 8, background: 'rgba(255,255,255,0.08)', borderRadius: 4, overflow: 'hidden' }}>
          <div style={{
            ...
            background: grGold(), borderRadius: 4, transition: 'width 0.6s ease',
          }} />
        </div>
      </div>
      ...
      {/* Badges */}
      <div>
        <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', color: C.muted, margin: '0 0 10px' }}>
          BADGES · {BADGES.filter(b => b.check(badgeState)).length}/{BADGES.length}
        </p>
          ...
          {BADGES.map(badge => {
            ...
            return (
              <div key={badge.id} style={{
                ...
                <div style={{ fontSize: 28, filter: earned ? 'none' : 'grayscale(1)' }}>
                  {badge.emoji}
                </div>
                <div style={{ fontSize: 12, fontWeight: 700, color: 'white' }}>{badge.name}</div>
                <div style={{ fontSize: 10, color: C.muted, lineHeight: 1.3 }}>{badge.desc}</div>
                {earned && (
                  <div style={{ fontSize: 9, fontWeight: 700, color, letterSpacing: '0.08em', marginTop: 2, textTransform: 'uppercase' }}>
                    {badge.rarity}
                  </div>
                )}
              </div>
      ...
      {/* Leaderboard button */}
      <button onClick={() => go(14)} style={{
        background: C.glass, border: `1px solid ${C.glassBdr}`, borderRadius: 16,
        padding: '16px', display: 'flex', alignItems: 'center',
        justifyContent: 'space-between', cursor: 'pointer', width: '100%', fontFamily: 'inherit',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 20 }}>🏆</span>
          <span style={{ fontSize: 14, fontWeight: 700, color: 'white' }}>Leaderboard</span>
        </div>
      ...
      <button onClick={() => go(13)} style={{
        background: 'linear-gradient(90deg, rgba(245,158,11,0.15), rgba(252,211,77,0.1))',
      ...
      <button onClick={() => go(7)} style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 700, color: 'white' }}>Settings</span>
        </div>
        <span style={{ color: C.muted, fontSize: 16 }}>›</span>
      </button>
