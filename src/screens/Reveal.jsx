...
export default function Reveal({ optA, optB, answers, user, go }) {
  // phase: 'counting' → 'draw_flip' (if draw) → 'result'
  const [phase, setPhase] = useState('counting');
  const [saved, setSaved] = useState(false);

  // Generate votes from algorithm using question answers
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      if (isDraw) {
        // Show coin flip first
        setPhase('draw_flip');
        setTimeout(async () => {
          // Coin randomly picks winner
  ...
  return (
    <div style={{
      ...
      background:    `radial-gradient(ellipse 90% 60% at 50% 30%,
        rgba(139,92,246,0.25) 0%, ${C.bg} 65%)`,
    }}>
      ...
      {phase === 'result' && (
        <div style={{
          ...
          {/* Loser — faded with red strikethrough */}
          <div style={{
            position: 'relative',
